...
顶部导航由 `assets/css/site-navigation.css` 和 `assets/js/site-navigation.js` 统一维护。新增二级页时，应复用 `.nav`、`#site-nav`、`.nav-actions` 结构，并同时引入这两个共享文件；不要在页面内另行定义导航视觉或菜单内容。

二级页 Hero 默认使用“中文标题在上、英文标题在下、统一检索框随后”的结构；教材资源库为既有例外。新增二级页应复用该结构与对应的检索行为。
